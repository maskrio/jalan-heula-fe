...
const ArticleCard = React.forwardRef<HTMLDivElement, ArticleCardProps>(
	({ article }, ref) => {
		...
		const date = new Date(
			...
			<Link href={`/articles/${encodeURIComponent(article.title || "")}`} passHref className="block pb-4">
				<div
					ref={ref}
				...
				>
					...
					{/* Text content */}
					<div className="flex-1 min-w-0"><div className="mb-3 flex items-center gap-2 text-xs text-muted-foreground font-medium">
							{category && (
						...
						</p>						<div className="flex items-center gap-6 text-xs text-muted-foreground mt-4 border-t pt-4 border-border/50">
